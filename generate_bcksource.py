...
pars = argparse.ArgumentParser(description='Generate the source files for the background, depending on the cutoff value, solar activity,altitude, inclination and other parameters')

pars.add_argument('LEObckpath',help=' full path where the scripts of Cumani are')
pars.add_argument('Outputpath',help='full path where the source files will be created')
...
pars.add_argument("-o",'--orientationfile',default=None,help='full path of orientation file [None]')

pars.add_argument("-la", "--latitude" ,default=None,type=float,help='latitude (deg) [None]')

pars.add_argument( "-lon","--longitude" ,default=None ,type=float,help='longitude (deg)[None]')           

pars.add_argument( '-d',"--date" ,default=None,type=float,help='date (MJD)[None]')


pars.add_argument("-c","--cutoff" ,default=10.,type=float,help='cut off rigidity (GeV) [10.]')

pars.add_argument("-g","--geomlat" ,default=-0.1,type=float,help='geomagnetic lat (rad) [-0.1]')

pars.add_argument("-mc","--mcosima" ,default=None,type=int,help='number of core to use for mcosima [None]')
...
if date and latitude and longitude and altitude is not None :
    ...
    geocordinate = np.array(aacgmv2.get_aacgm_coord(latitude, longitude, altitude, t_bin))
    ...
    cutoff = (M/4*(1+altitude/EarthRadius)**(-2.0)* np.cos(np.deg2rad(geocordinate[0]))**4)
...
#generate the .dat input spectrum file for MEGALib (by default altitude is 550km, solar mod 650MV and inclination 0 deg)

os.chdir(LEObckpath)
os.system("python CreateBackgroundSpectrumMEGAlib.py -c {0:.3f} -i {1} -a {2} -s {3} -o {4} -g {5}".format(cutoff,inclination,altitude,solarmodulation,outputpath,geomlat))
...
#get the flux for each component in  #/cm^2/s
for p in Particle :
    Infile = open(outputpath+"/"+p+"_Spec_{0}km_{1}deg_{2:.3f}cutoff_{3}solarmod.dat".format(altitude,inclination,cutoff,solarmodulation),"r")
    lines = Infile.readlines()
    flux.append(float(lines[4].split()[3]))
    
df = pd.DataFrame({'Particle':Particle, 'flux':flux })
...
#Primary electron
f = open("PrimaryElectron.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n")
...
f.write("SecondaryElectrons.Flux		      {0}\n\n".format(df['flux']['SecondaryElectrons']))
f.write("SecondaryElectron.LightCurve File false {0}\n".format(Lightcurvepath+"/SecondaryElectrons_LightCurve_corrected.dat"))
f.close()

 
 
#Primary Positron
f = open("PrimaryPositron.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n")
f.write("PhysicsListHD   qgsp-bic-hp\n")
...
f.write("PrimaryPositron.LightCurve File false {0}\n".format(Lightcurvepath+"/PrimaryPositrons_LightCurve_corrected.dat"))
f.close()     
         
#Secondary Positron
f = open("SecondaryPositron.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n")
f.write("PhysicsListHD   qgsp-bic-hp\n")
...
f.write("SecondaryPositrons.LightCurve File false {0}\n".format(Lightcurvepath+"/SecondaryPositrons_LightCurve_corrected.dat"))
f.close()              



#Cosmic Photons      
f = open("CosmicPhoton.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n") 
f.write("PhysicsListEM LivermorePol\n\n")
...
f.write("CosmicPhotons.Beam	FarFieldFileZenithDependent CosmicPhotonsGruber.beam.dat\n")
f.write("CosmicPhotons.Spectrum	File CosmicPhotons_Spec_{0}km_{1}deg_{2:.3f}cutoff_{3}solarmod.dat\n".format(altitude,inclination,cutoff,solarmodulation))
f.write("CosmicPhotons.Flux		   {0}\n\n".format(df['flux']['CosmicPhotons']))
f.close()	       
     
#Albedo Photons      
f = open("AlbedoPhoton.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n") 
f.write("PhysicsListEM LivermorePol\n")
...
f.write("AlbedoPhotons.Flux	  {0}\n\n".format(df['flux']['AlbedoPhotons']))
f.write("AlbedoPhotons.LightCurve File false {0}\n".format(Lightcurvepath+"/AlbedoPhotons_LightCurve_corrected.dat"))
f.close()

#Neutron Atm (step 1)
f = open("NeutronAtmospheric_step1.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n") 
f.write("PhysicsListHD   qgsp-bic-hp\n")
...
f.write("NeutronAtm.Flux   {0}\n\n".format(df['flux']['AtmosphericNeutrons']))
f.write("NeutronAtm.LightCurve File false {0}\n".format(Lightcurvepath+"/AtmosphericNeutrons_LightCurve_corrected.dat"))
f.close()

#Neutron Atm (step 2)
f = open("NeutronAtmospheric_step2.source","w")         
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant               0.000002\n") 
f.write("PhysicsListHD                      qgsp-bic-hp\n")
f.write("PhysicsListEM                      LivermorePol\n")
...
if mc==None :
    ...
    for i in range(1,mc+1):
        f.write("A.IsotopeProductionFile   NeutronAtmIsotopes.p1.inc{0}.dat\n".format(i))


f.close()

#Neutron Atm (step 3)
f = open("NeutronAtmospheric_step3.source","w")  
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant               0.000002\n")
f.write("PhysicsListHD                      qgsp-bic-hp\n")
f.write("PhysicsListEM                      LivermorePol\n")
...
f.write("ActivationStep3.Time                             {0}\n".format(time))
f.write("ActivationStep3.ActivationSources                NeutronAtm_Activation.dat\n")
f.close() 
...
f.write("SecondaryProtons.Beam  FarFieldFileZenithDependent AlbedoProtonMizuno.beam.dat\n")
f.write("SecondaryProtons.Spectrum File SecondaryProtonsUpward_Spec_{0}km_{1}deg_{2:.3f}cutoff_{3}solarmod.dat\n".format(altitude,inclination,cutoff,solarmodulation))
f.write("SecondaryProtons.Flux                 {0}\n\n".format(df['flux']['SecondaryProtonsUpward']))
f.write("SecondaryProtons.LightCurve File false {0}\n".format(Lightcurvepath+"/SecondaryProtonsUpward_LightCurve_corrected.dat"))
f.close()
...
if mc==None :
    ...
    for i in range(1,mc+1):
        f.write("A.IsotopeProductionFile   SecondaryProtonsIsotopes.p1.inc{0}.dat\n".format(i))
...
f.write("PrimaryProtons.Beam FarFieldFileZenithDependent CosmicProtonsSpenvis.beam.dat\n")
f.write("PrimaryProtons.Spectrum  	     File PrimaryProtons_Spec_{0}km_{1}deg_{2:.3f}cutoff_{3}solarmod.dat\n".format(altitude,inclination,cutoff,solarmodulation))
f.write("PrimaryProtons.Flux		     {0}\n\n".format(df['flux']['PrimaryProtons']))
f.write("PrimaryProtons.LightCurve File false {0}\n".format(Lightcurvepath+"/PrimaryProtons_LightCurve_corrected.dat"))
f.close()
...
if mc==None :
    ...
    for i in range(1,mc+1):
        f.write("A.IsotopeProductionFile   PrimaryProtonsIsotopes.p1.inc{0}.dat\n".format(i))
...
f.write("ActivationStep3.Time                             {0}\n".format(time))
f.write("ActivationStep3.ActivationSources                PrimaryProtons_Activation.dat\n")
f.close() 
...
f = open("PrimaryAlpha_step1.source","w")
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant 0.000002 \n")
f.write("StoreSimulationInfo  init-only \n") 
f.write("PhysicsListHD   qgsp-bic-hp\n")
...
f.write("PrimaryAlpha.Beam FarFieldFileZenithDependent  CosmicAlphasSpenvis.beam.dat\n")
f.write("PrimaryAlpha.Spectrum File PrimaryAlphas_Spec_{0}km_{1}deg_{2:.3f}cutoff_{3}solarmod.dat\n".format(altitude,inclination,cutoff,solarmodulation))
f.write("PrimaryAlpha.Flux		    {0}\n\n".format(df['flux']['PrimaryAlphas']))
f.write("PrimaryAlpha.LightCurve File false {0}\n".format(Lightcurvepath+"/PrimaryAlpha_LightCurve_corrected.dat"))
f.close()    
         
#Primary Alpha (step 2)
f = open("PrimaryAlpha_step2.source","w")         
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant               0.000002\n") 
f.write("PhysicsListHD                      qgsp-bic-hp\n")
f.write("PhysicsListEM                      LivermorePol\n")
...
if mc==None :
    ...
    for i in range(1,mc+1):
        f.write("A.IsotopeProductionFile   PrimaryAlphaIsotopes.p1.inc{0}.dat\n".format(i))  

f.close()

#Primary Alpha (step 3)
f = open("PrimaryAlpha_step3.source","w")  
f.write("Geometry {0} \n".format(geopath))
f.write("DetectorTimeConstant               0.000002\n")
f.write("PhysicsListHD                      qgsp-bic-hp\n")
f.write("PhysicsListEM                      LivermorePol\n")
...
f.write("ActivationStep3.Time                             {0}\n".format(time))
f.write("ActivationStep3.ActivationSources                PrimaryAlpha_Activation.dat\n")
f.close() 
